...
class StructureValidator {
  ...
  countCodeLines(filePath) {
    ...
    for (const line of lines) {
      ...
      if (filePath.endsWith('.css') && trimmed.startsWith('/*') && trimmed.endsWith('*/')) {
        continue;
      }
  ...
  validateE2ETests() {
    ...
    files.forEach(file => {
      ...
      if (isStyles && !firstLine.includes('@agent: styles')) {
        this.errors.push(`${file}: Must start with '// @agent: styles' metadata comment`);
      }
      if (isBug && !firstLine.includes('@agent: frontend-debugger')) {
        this.errors.push(`${file}: Must start with '// @agent: frontend-debugger' metadata comment`);
      }
    ...
    components.forEach(component => {
      ...
      RULES.components.structure.required.forEach(required => {
        if (!files.includes(required)) {
          this.errors.push(`Component ${component}: Missing required file ${required}`);
        }
      });
      ...
      if (fs.existsSync(indexPath)) {
        this.validateFileLength(indexPath, RULES.lineLimits.component, 'component');
      }

      // Check CSS module naming and length
      const cssModules = files.filter(f => f.endsWith('.module.css'));
      if (cssModules.length > 1) {
        this.errors.push(`Component ${component}: Multiple CSS modules found. Should have only one.`);
      }
      if (cssModules.length === 1) {
        const cssFile = cssModules[0];
        if (cssFile !== `${component}.module.css`) {
          this.errors.push(`Component ${component}: CSS module should be named ${component}.module.css`);
        }
      ...
      if (fs.existsSync(testsPath)) {
        ...
        tests.forEach(test => {
          if (!test.endsWith('.test.ts')) {
            this.errors.push(`Component ${component}: Invalid test file ${test}. Must end with .test.ts`);
          }
          if (test.endsWith('.spec.ts')) {
            this.errors.push(`Component ${component}: Playwright tests (.spec.ts) should be in e2e/, not component folder`);
          }

          // Validate test file length
          const testPath = path.join(testsPath, test);
          if (fs.existsSync(testPath) && test.endsWith('.test.ts')) {
            this.validateFileLength(testPath, RULES.lineLimits.test, 'unit test');
          }
        });
      }
    });
  ...
  validateTestSeparation() {
    ...
    specFiles.forEach(file => {
      this.errors.push(`Playwright test found in component folder: ${file}\n  Should be in e2e/ directory`);
    });
  }
  ...
  generateReport() {
    ...
    const reportPath = path.join(process.cwd(), 'structure-validation-report.json');
