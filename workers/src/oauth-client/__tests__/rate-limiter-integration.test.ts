...
describe('OAuth Rate Limiting Integration', () => {
  ...
  describe('OAuth callback rate limiting', () => {
    it('should allow requests up to the rate limit', async () => {
      ...
      const blockedRequest = new Request('http://localhost/oauth/callback?code=test&state=test&code_verifier=test', {
        headers: {
          'CF-Connecting-IP': clientIp
        }
      });
  ...
  describe('Performance', () => {
    it('should handle rate limit checks in less than 50ms', async () => {
      const request = new Request('http://localhost/oauth/callback?code=test&state=test&code_verifier=test', {
        headers: {
