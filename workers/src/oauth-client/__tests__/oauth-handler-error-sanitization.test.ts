...
describe('OAuth Handler Error Sanitization', () => {
  ...
  it('should return generic error message when code is missing', async () => {
    const request = new Request('https://example.com/callback?state=test-state&code_verifier=test-verifier');
  ...
  it('should return generic error message when state is missing', async () => {
    const request = new Request('https://example.com/callback?code=test-code&code_verifier=test-verifier');
    const response = await handleOAuthCallback(request, mockEnv, mockContext);
  ...
  it('should return generic error message for invalid state format', async () => {
    const request = new Request('https://example.com/callback?code=test-code&state=invalid!@#$%^&*()state&code_verifier=test-verifier');
    const response = await handleOAuthCallback(request, mockEnv, mockContext);
  ...
  it('should return generic error message when session is not found', async () => {
    ...
    const request = new Request('https://example.com/callback?code=test-code&state=test-state&code_verifier=test-verifier');
    const response = await handleOAuthCallback(request, mockEnv, mockContext);
