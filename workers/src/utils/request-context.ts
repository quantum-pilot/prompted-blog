...
export class RequestContext {
  ...
  static async create(request: Request, _env: any): Promise<RequestContext> {
    ...
    const extractedId = HeaderUtils.extractCorrelationId(request, url.searchParams.get('state'));
  ...
  log(eventType: AuditEventType, result: 'success' | 'failure', metadata?: Record<string, any>): void {
    this.logger.log(eventType, result, {
      correlationId: this.correlationId,
      userId: this.userId,
      sessionId: this.sessionId,
      ipAddress: this.ipAddress,
      userAgent: this.userAgent,
    }, metadata, this.metadataManager.getAll());
  }
