...
describe('Data Access Decorator', () => {
  ...
  const applyDecorator = (method: any, resource: string, methodName: string) => {
  ...
  it('should log successful sync method execution', async () => {
    ...
    const originalMethod = vi.fn((context: DataAccessContext) => ({
      id: context.requestContext.userId, name: 'Test User'
    }));
    const modifiedDescriptor = applyDecorator(originalMethod, 'UserData', 'getUser');
    ...
    expect(AuditLogger.logDataAccess).toHaveBeenCalledWith(
      'user123', 'UserData.getUser', 'read', true
    );
  });
  ...
  it('should log successful async method execution', async () => {
    ...
    const originalMethod = vi.fn(async (context: DataAccessContext, data: any) => ({
      success: true, userId: context.requestContext.userId
    }));
    const modifiedDescriptor = applyDecorator(originalMethod, 'UserData', 'updateUser');
    const context: DataAccessContext = {
      requestContext,
      resource: 'user-profile',
      operation: 'write'
    ...
    expect(AuditLogger.logDataAccess).toHaveBeenCalledWith(
      'user456', 'UserData.updateUser', 'write', true
    );
  });
  ...
  it('should log failed method execution and rethrow error', async () => {
    ...
    const modifiedDescriptor = applyDecorator(originalMethod, 'SecureData', 'deleteItem');
    const requestContext = new RequestContext();
    ...
    await expect(modifiedDescriptor.value(context)).rejects.toThrow('Access denied');
    
    expect(originalMethod).toHaveBeenCalledWith(context);
    expect(AuditLogger.logDataAccess).toHaveBeenCalledWith(
      'user789', 'SecureData.deleteItem', 'delete', false
    );
  });

  it('should warn and skip logging when context is missing', async () => {
    const originalMethod = vi.fn(() => 'data');
    const modifiedDescriptor = applyDecorator(originalMethod, 'TestData', 'getData');

    const result = await modifiedDescriptor.value('no-context');
    
    expect(result).toBe('data');
    expect(originalMethod).toHaveBeenCalledWith('no-context');
    expect(consoleWarnSpy).toHaveBeenCalledWith('Missing audit context for TestData.getData');
