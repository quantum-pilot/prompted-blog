...
export class AuditLogger {
  ...
  private static determineLogLevel(eventType: AuditEventType, result: 'success' | 'failure'): LogLevel {
    if (eventType === AuditEventType.AUTH_LOGIN_FAILURE ||
  ...
  static logAuthSuccess(userId: string, sessionId: string, request: Request): void {
  ...
  static logAuthFailure(attemptedEmail: string, reason: string, request: Request): void {
  ...
  static logSessionCreated(userId: string, sessionId: string): void {
    this.log(AuditEventType.AUTH_SESSION_CREATED, 'success', { userId, sessionId });
  }

  static logSessionDestroyed(userId: string, sessionId: string): void {
    this.log(AuditEventType.AUTH_SESSION_DESTROYED, 'success', { userId, sessionId });
  }

  static logDataAccess(userId: string, resource: string, action: string, success: boolean): void {
    const eventMap = {
      'read': success ? AuditEventType.DATA_ACCESS_READ : AuditEventType.DATA_ACCESS_DENIED,
      'write': success ? AuditEventType.DATA_ACCESS_WRITE : AuditEventType.DATA_ACCESS_DENIED,
      'delete': success ? AuditEventType.DATA_ACCESS_DELETE : AuditEventType.DATA_ACCESS_DENIED,
    };
    
    const eventType = eventMap[action as keyof typeof eventMap] || AuditEventType.DATA_ACCESS_READ;
    this.log(eventType, success ? 'success' : 'failure', { userId, resource, action });
  }
}